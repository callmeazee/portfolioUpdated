...
| ISS-050 | Architecture diagrams for the other projects | 🟡 | Low | Snitcher and Besties have diagrams derived from their source. CloudSpire, ConnectVerse, FileMoon and MoviePlas carry `diagram: null` — deliberately, since their architecture has not been read |
...
| ISS-009 | Case study depth | 🟡 | High | **Snitcher done** — architecture, frontend, backend, database, infrastructure and security written from its repository source, every claim traceable to a file. `problem`, `challenges` and `learnings` stay empty there: they are the author's account and cannot be derived from code. Other projects still at overview + features |
| ISS-044 | Besties repository is private | 2026-09-08 — made public. Case study written from source; live link `besties-sandy.vercel.app` recovered from the repo metadata. Now featured |
| ISS-046 | Besties: peer is not told when a call drops | 🔴 | High | **Found while reading the source.** In `video.socket.ts` the `disconnect` handler only logs — it never emits `end` to the other party, so when one side drops the other sits waiting indefinitely. This matches the reported "not responding" symptom exactly. Fix: on disconnect, emit `end` to any peer in an active call |
| ISS-047 | Besties: chat sender gets no echo | 🟡 | High | `chat.socket.ts` emits `message` only to `payload.to`, never back to the sender. If the UI waits on the socket to render an outgoing message, the sender sees nothing happen — the second half of the reported symptom. Fix: emit to both rooms, or render optimistically on send |
| ISS-048 | Besties: signalling addresses mix user ids and socket ids | 🟡 | Medium | `offer` is sent to a user-id room and carries `from.socketId`, while `answer` and `candidate` reply with `from: socket.id`. Mixing the two makes ICE candidates easy to misroute across tabs or reconnects. Worth settling on user ids throughout |
| ISS-049 | Besties: presence is per-process and in-memory | 🟡 | Low | `onlineUsers` is a `Map` in module scope, so it resets on restart and cannot work across more than one instance. Fine at current scale; would need Redis to scale out |
| ISS-045 | Snitcher CORS allows every origin | 🔴 | High | **Found while reading the source.** `Backend/src/app.js` builds an allow-list of origins, but every branch — including the fallback — calls `callback(null, true)`, so all origins are accepted while appearing restricted. Combined with `credentials: true` and cookie auth, any site can make credentialed requests against the API. Fix is in the snitcher repo: return `callback(new Error(...))` on no match |
...
| ISS-038 | Besties details need confirming | 2026-09-08 — resolved from source. The real stack is broader than the README example: TypeScript throughout, Twilio for ICE, S3 presigned URLs, refresh-token rotation, Swagger, SWR |
