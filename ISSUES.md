...
| ISS-040 | MoviePlas is broken in production | 🔴 | High | Its TMDB integration fails on load — "Failed to load home content", zero images, reproduced on two separate days. The portfolio links to it, so a visitor sees a broken app. Worth fixing or delisting |
